...
<div class="about-hero">
  <p class="tagline">Prime MMEL , the torch carried forward.</p>
</div>
...
Greek *protos* (first, original) , both ultimately from a root
...
The Primmel logo is a custom-drawn mark , a stylised monogram with
...
<div class="logo-breakdown">
  ...
  <div class="description">
    ...
    <p>
      ...
      Primmel is for standards work , serious, crafted, lasting.
    </p>
    <h3>Indigo , the Brand Colour</h3>
    <p>
...
<div class="torch-legend">
  ...
  <div><strong>Indigo , #1e3a8a</strong><span>The light-mode mark. Deep, formal, contemplative , the colour of a standard taken seriously.</span></div>

  <div class="torch-swatch indigo-light"></div>
  <div><strong>Sky Indigo , #93c5fd</strong><span>The dark-mode mark. Lifts the tonal values so the mark stays luminous against dark surfaces.</span></div>

  <div class="torch-swatch slate"></div>
  <div><strong>Slate , #0f172a</strong><span>The dark-mode background anchor. The darkest tonal values in the mark resolve to slate on dark themes so they fade into the surface.</span></div>

  <div class="torch-swatch paper"></div>
  <div><strong>Paper , #ffffff</strong><span>The light-mode background. The lightest tonal values in the mark resolve to white on light themes.</span></div>
</div>
...
between them , all defined in one place and consumable by tools.
...
apply to legal metrology certification , the publication and
dissemination of requirements, conformance tests, and test report forms
, from test laboratories to evaluation agencies, manufacturers,
...
that adopts the standard publishes its implementation model , the
...
<div class="use-cases-grid">
  <div class="use-case-card">
    ...
    <p>Publish standards in a form readers can navigate, integrate, and test against , not only read.</p>
  </div>
  <div class="use-case-card">
...
- **Data primitives** , classes, enums, references, with cardinality
  (a la UML).
- **Process primitives** , start events, end events, gateways,
  processes, canvases (a la BPMN).
- **Compliance primitives** , provisions with RFC 2119 modality
  (SHALL, SHOULD, MAY, CAN, MUST).
- **Measurement primitives** , numeric, datalist, derived values
  for quantitative monitoring.
- **Mapping primitives** , links and alignments between models
  (`.prm` files).
- **Evidential primitives** , hooks for attaching evidence to any
...
- **OIML SMART** , international legal metrology certification
  (publication, conformance tests, test report forms).
- **BSI SMART** , the British Standards Institution's SMART program
  for executable standards.
- **QMS, ISMS, MDSAP, ISO 13485, ISO 14971, ISO 27001** , quality,
  information security, and medical-device management systems modelled in
  the legacy MMEL.
- **BS 20400, BS 44003, BS 16341** , British Standards modelled as
...
- **Examples**: real models from the MMEL legacy, kept as reference and
  learning material.
- **Tools**: editors, validators, and runtime support , all
  community-driven.
...
1. **Read the introduction** , start with
   [Introduction](/docs/introduction).
2. **See a first model** , work through
   [A First Model](/docs/first-model).
3. **Browse the examples** , real MMEL models converted to Primmel,
   in [Examples](/examples/).
4. **Read the full specification** , the current
   [Primmel Language Specification, v3](https://www.primmel.org/spec/language-v3/),
   with the v2 edition (MN 113) at
   [primmel.org/spec](https://www.primmel.org/spec/).
5. **Try a model** , load a model in your editor of choice and
