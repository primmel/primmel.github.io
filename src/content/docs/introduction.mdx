...
- **Mappings** , declare how an implementation model aligns to a
  reference model, element by element.
- **References** , cite external document clauses (`.prd` extracts,
  PDF clauses, etc.) that provisions trace back to.
- **Approvals** , bind a step to an approver role and an approval
  record registry.
...
order , `role`, `process`, `provision`, `class`, `data_registry`,
...
- **A diagram** , render the canvas as a flowchart using the
  `x`/`y` coordinates and the `process_flow` edges.
- **A data schema** , compile every `class#data` and its fields
  into a form a database or API can validate against.
- **A compliance matrix** , list every `provision`, its
  `modality`, the `reference` it traces back to, and the processes that
  `validate_provision` it.
- **A test harness** , evaluate `validate_measurement` expressions
  against actual records in a `.pws` workspace file.
- **A mapping table** , given a `.prm` file, render a side-by-side
...
- [A First Model](/docs/first-model) , build the minimal model
  from scratch, line by line.
- [Examples](/examples/) , the curated example corpus, each
  file dissected.
- [Data primitives](/docs/data-model), [process primitives](/docs/process-model),
  ...
  [mapping](/docs/mapping) , focused reference for each pillar.
- The canonical specification ,
  [Primmel Language Specification, v3](https://www.primmel.org/spec/language-v3/)
  (the v2 edition, MN 113, remains at
