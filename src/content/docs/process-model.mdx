...
typically has multiple `start_event`s , one per diagram. There
can be more `end_event`s than `start_event`s , a flow can
...
- `validate_provision { ... }` , the provisions it must satisfy.
  See [compliance](/docs/compliance).
- `validate_measurement { ... }` , the measurement expressions
  that must hold.
...
- `process.parent` , the ID of the parent process (empty string
  for top-level processes)
- `process.children` , an array of child process IDs

Nesting is **additive** , existing flat models still work. A
model with no nesting has empty `children` arrays and empty
`parent` strings.
...
- [Example 01: Minimal model](/examples/minimal-model) , the
  smallest valid canvas.
- [Example 03: Process flow](/examples/process-flow) ,
  gateways, timers, multi-page canvases.
- [Example 05: Approval workflow](/examples/approval-workflow) ,
