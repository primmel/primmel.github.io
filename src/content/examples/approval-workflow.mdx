...
This example introduces the `approval` step , a specialised
...
The `role` type for `approved_by` is a *role-typed field* , it
...
## The reason for a separate primitive
...
- The `approve_by` clause is distinct from the `actor` clause ,
