...
import ImplementationPackageDiagram from '../../components/diagrams/ImplementationPackageDiagram.astro';
...
13485, ISO 14971, ISO 27001, MDSAP, and other major standards ,
with invented content suitable for public distribution.

<ImplementationPackageDiagram />
...
- `.prd` , the *source of truth* for what the standard says.
- `.prl` , the *implementation* model.
- `.prm` , the *alignment* between implementation and standard.
- `.pws` , the *evidence* , actual records produced when
...
## Pattern: multi-page canvases

The model has four canvases:

| Canvas | Diagram contents |
| --- | --- |
...
Each canvas is a separate diagram with its own `start_event`. The
...
wait 12 months, review again"* , indefinitely.
...
description/justification in the `.prm` file , this is normal
...
1. Start with [`ocs-standard.prd`](/examples/files/06-implementation-package/ocs-standard.prd).
   Note the clause numbers (`4.1`, `4.2`, `4.3`, `4.4`) , these
...
6. Open [`sample-workspace.pws/`](/examples/files/06-implementation-package/sample-workspace.pws/manifest.yaml).
   ...
   Its `attributes` map onto the fields of `class RoastBatch#data` ,
