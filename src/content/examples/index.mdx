...
> **Now speaking Primmel v3** (a strict superset of v2): the twin
> constructs (`endpoint`/`serve`, a subject's declared API surface),
> the composition facet (`composed_of`, hierarchical twins with the
> closed state rule), and the monitor declaration (continuous
> compliance, triggers and escalations). The examples below are the v2
> foundations they extend, every one still parses and runs as
> written; the v3 constructs build on exactly these primitives.
> The live-twin chapter of the Primmel volume carries the v3 surface
> ([the docs federation](https://primmel.github.io/primmel-smart-docs/primmel/14-live-twins/)).
...
is **entirely fictional** , no real standards, organisations, or
proprietary content appears anywhere. The patterns the examples
exercise (cross-model aliasing, multi-page canvases, subject
...
An order-fulfillment flow with two canvas diagrams on the same
model. Shows how `start_event`s, `timer_event`s, and
`exclusive_gateway`s combine with the `"default"` fallback edge.

- Demonstrates: multi-page canvases, multiple `start_event`s,
...
- Demonstrates: the full Primmel workflow , `.prl` + `.prd` +
...
- Canvases are named `Root` (the top-level flow) plus page-style
  names (`BeanSourcingFlow`, `QualityCheckFlow`).
- Multiple `start_event`s are normal , one per canvas diagram.
...
Once you understand the patterns above, see [Real-world
scale](/examples/real-world-scale) for a structural overview of how
real standards models look in Primmel , model sizes, canvas
counts, and provision density for ISO 27001, BS 44003, and other
standards. (The full models are in a private repository; only
aggregate statistics and structural patterns are shown publicly.)