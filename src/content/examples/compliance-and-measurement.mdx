...
- A `condition` , the human-readable requirement.
- A `modality` , `SHALL`, `SHOULD`, `MAY`, `CAN`, or `MUST` (RFC
  2119).
- A `reference { ... }` block , the clauses this provision
  derives from.
...
- A `description` , what a human reads on the diagram.
- A `condition` , a measurement expression that the runtime
...
- **Cross-model aliasing** , provisions live entirely inside this
  file. Cross-model aliasing is added in the
  [implementation package](/examples/implementation-package).
- **Workspace data** , the model declares measurements but does
