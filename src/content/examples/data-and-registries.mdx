...
roastery logbook , bean lots received at the dock, batches
roasted, batches inspected , with the registries that hold those
records.
...
The `#data` suffix marks this class as registerable , you can put
...
classes. It cannot be registered directly , there is no
...
these references to traverse the data graph , e.g. from a
...
| --- | --- | --- |
| `ReceiveBeans` | , | `BeanLotRegistry` |
| `RoastBatch` | `BeanLotRegistry` | `RoastBatchRegistry` |
| `InspectBatch` | `RoastBatchRegistry` | , |
...
- **Compliance** , no `provision`, no `modality` on processes.
  Added in [compliance and measurement](/examples/compliance-and-measurement).
- **Branching** , the flow is a straight line. Branching is
  added in [process flow](/examples/process-flow).
- **Subject bindings** , provisions that evaluate against
