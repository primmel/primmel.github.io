...
This example models an order-fulfillment flow with two **canvas
...
(`timer_event`), and the multi-page canvas pattern that real-world
models rely on.
...
| Primitive | Purpose |
| --- | --- |
| Multiple `start_event`s | One entry point per canvas diagram. |
...
A model with *N* canvas diagrams typically has *N* `start_event`s.
Each diagram has exactly one entry point. There can be more
`end_event`s than `start_event`s , a single flow can terminate in
...
A `REPEAT` timer is the engine of recurring processes , see
...
The `canvas OrderFlow` clause binds the process to a named diagram.
This is how a model declares which canvas a process "lives on" when
...
- **`StockCheck`** , in stock goes left to payment; out of stock
  goes right to cancellation.
- **`PaymentConfirmed`** , paid goes left to fulfilment; unpaid
...
- **Compliance** , no `provision` or `modality`. Added in
  [compliance and measurement](/examples/compliance-and-measurement).
- **Data** , the `data { }` blocks are empty. Data appears in
  [data and registries](/examples/data-and-registries).
- **Machine-readable conditions** , edges use only `description`.
