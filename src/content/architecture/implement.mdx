...
implementation model is the *starting point* , not a blank slate.
...
- **Local namespace** , `metadata.namespace` is the organisation's
  own (e.g. `AcmeCoffeeProgramme`), not the standard's.
- **Local processes and data** , processes, classes, registries
...
quality testing. There is no reference to any standard here yet ,
...
- Implicit: a Statement of Applicability, derivable from the mapping by
  tooling , "every reference element → ≥1 implementation element".
...
- [Example 02: Data and registries](/examples/data-and-registries)
  , a small org's implementation of a roastery logbook, with no
  reference to any standard.
- [Implementation package walkthrough](/examples/implementation-package)
  , the showcase. Acme adopts the OCS standard, modifies its
  implementation as needed, builds the mapping, and closes it.
- [Implementation package , `acme-coffee-programme.prl`](/examples/files/06-implementation-package/acme-coffee-programme.prl)
  , Acme's full digital twin, before/after adopting the OCS
  standard.
- [Mapping](/docs/mapping) , the dual forms of mapping in detail.

## After implementing

The implementation model is maintained continuously , whenever
