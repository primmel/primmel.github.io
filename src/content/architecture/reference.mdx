...
# Reference , read, interact, and evaluate a published standard

<div class="callout">

**WHO THIS IS FOR**

The **Reference** pillar is for **Readers** , anyone who needs to
...
- **Interact** , exercise the standard dynamically: run the test
  processes it declares, follow its process flows, evaluate its
  measurement expressions against sample inputs.
- **Evaluate** , check whether a product, process, or dataset
...
| Interact | A test run , executed process flows, evaluated formulas |
...
| Artifact | Purpose |
| --- | --- |
| Published reference `.prl` | The standard itself , provisions, processes, data classes, references |
...
- **Test harnesses** , execute the standard's test processes
...
- They do not audit implementations against the standard. That's the
  [Audit](/architecture/audit) pillar , which *uses* reader-style
...
- [Compliance and measurement example](/examples/compliance-and-measurement)
  , the gateway edges with `condition` expressions and the
...
reviews. But the Reader activity itself stops at the verdict ,
